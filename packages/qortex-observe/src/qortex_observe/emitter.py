...
def configure(config: "ObservabilityConfig | None" = None) -> EventEmitter:
    ...
    from qortex_observe.subscribers.structlog_sub import register_structlog_subscriber

    register_structlog_subscriber()

    # Always register JSONL event sink if path configured
    if cfg.jsonl_path:
        from qortex_observe.subscribers.jsonl import register_jsonl_subscriber
    ...
    if cfg.otel_enabled:
        try:
            from qortex_observe.subscribers.otel import register_otel_traces
    ...
    from qortex_observe.subscribers.alert import register_alert_subscriber
