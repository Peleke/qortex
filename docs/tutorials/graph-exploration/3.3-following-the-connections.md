...
```mermaid
graph LR
    IM["Immutability"] -- CONTRADICTS --> DS["Dangerous Setters"]
    DS -- CHALLENGES --> EN["Encapsulation"]
    DS -- ENABLES --> IS["InvalidState"]
```
...
graph TD
    ...
    DS -- CHALLENGES --> EN["Encapsulation"]
    DS -- ENABLES --> IS["InvalidState"]
    IS -- CAUSES --> PB["ProductionBug"]
    IS -- VIOLATES --> CI["Class Invariant"]
    EN -- REQUIRES --> PR["Properties"]

    style DS fill:#e74c3c,color:#fff
    style PB fill:#c0392b,color:#fff
```
