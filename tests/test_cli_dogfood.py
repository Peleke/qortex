...
def populated_backend():
    ...
    rules = [
        ExplicitRule(
            id="rule1", text="Always do X before Y", domain="test-domain", source_id="src1"
        ),
        ExplicitRule(
            id="rule2", text="Never combine A with B", domain="test-domain", source_id="src2"
        ),
    ]
    for rule in rules:
    ...
    def test_project_buildlog_unwritable_output(self, populated_backend):
        ...
        with patch("qortex.cli.project._get_backend", return_value=populated_backend):
            result = runner.invoke(
                app, ["project", "buildlog", "--output", "/root/unwritable.yaml"]
            )
